...
Затем задайте пароль — скрипт спросит его дважды и напечатает две строки:
...
Пароль нигде не сохраняется, восстановить его из хеша нельзя — только задать
новый тем же скриптом. Полученные строки плюс путь к папке данных положите
в `~/momo/.env.local`:

```bash
cat > ~/momo/.env.local <<'EOF'
MOMO_DATA_DIR=/home/momo/momo-data
ADMIN_PASSWORD_HASH=<строка из скрипта>
ADMIN_SESSION_SECRET=<строка из скрипта>
EOF
chmod 600 ~/momo/.env.local
```

Файл не в git — обновления сайта его не трогают. После этого пересоберите
(`bash scripts/deploy.sh`) и заходите на `https://momo-eq.ru/admin`.
