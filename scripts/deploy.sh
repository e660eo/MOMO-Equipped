...
# одна способна удалить файлы прямо во время работы другой. У deploy отдельный
# замок: auto-update.sh держит свой deploy.lock вокруг проверки git, а каждый
# способ запуска сборки дополнительно проходит через общий deploy-run.lock.
mkdir -p logs
exec 8>logs/deploy-run.lock
if ! flock -n 8; then
