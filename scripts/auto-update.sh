...
#
# Замок лежит рядом с проектом, а не в /tmp: общая папка доступна на запись
# любому пользователю системы, и подложенный туда заранее симлинк заставил
# бы `exec 9>` обрезать тот файл, на который он указывает, — от имени momo.
mkdir -p logs
