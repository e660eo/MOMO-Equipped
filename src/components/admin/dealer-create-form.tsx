...
export function DealerCreateForm({ initial = {} }: { initial?: DealerCreateInitial }) {
  ...
  return <form action={action} className="mt-5 grid gap-3 sm:grid-cols-2">
    ...
    <label className="grid gap-1 text-xs text-muted-foreground">Скидка от РРЦ, %<input className={input} type="number" min="0" max="80" step="0.1" name="discountPercent" defaultValue="0" required /></label>
