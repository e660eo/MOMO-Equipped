...
import { getDealerOrderAgreement, DEALER_PAYMENT_LABELS } from "@/lib/dealer-order-management";
...
export function DealerOrdersPanel({ orders }: { orders: DealerOrder[] }) {
  ...
  const notes = new Map(getDealerOrderNotes().map((item) => [item.orderId, item]));
  return (
    <section id="dealer-orders" className="mt-5 scroll-mt-40 rounded-xl border border-border bg-surface p-5">
      ...
      <div className="mt-5 space-y-3">{orders.map((order) => {
        ...
        return <article key={order.id} className="rounded-lg border border-border p-4"><div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="text-right"><p className="text-lg font-black">{formatPrice(agreement?.total ?? order.total)}</p><p className="mt-1 text-xs font-bold">{agreement ? DEALER_PAYMENT_LABELS[agreement.paymentStatus] : "Условия не согласованы"}</p>{agreement && <p className="mt-1 text-xs text-muted-foreground">Согласовано, с доставкой · версия {agreement.revision}</p>}{note?.followUpDate && <p className="mt-2 text-xs">Связаться: {note.followUpDate.split("-").reverse().join(".")}</p>}<form action={setDealerOrderStatus} className="mt-2 flex flex-wrap gap-2"><input type="hidden" name="id" value={order.id} /><input type="hidden" name="expectedStatus" value={order.status} /><select className={inputClass} aria-label={`Статус заказа ${order.id}`} name="status" defaultValue={order.status}><option value="new">Новый</option><option value="confirmed">Подтверждён</option><option value="shipped">Отгружен</option><option value="done">Выполнен</option><option value="canceled">Отменён</option></select><button className="min-h-11 rounded-md bg-foreground px-3 text-xs font-bold text-bg">Сохранить</button></form></div>
        </div></article>;
      })}{!orders.length && <p className="rounded-lg bg-black/[.025] p-6 text-center text-sm text-muted-foreground">Заказов по выбранным условиям нет.</p>}</div>
    </section>
  );
}
