...
import { isListeningStandProduct, hasPublishedListeningAudio } from "@/lib/listening-stand";
import { DEFAULT_SOCIAL_IMAGE } from "@/lib/seo-metadata";

export const metadata: Metadata = {
  title: "Онлайн-стенд автозвука MOMO и ZEUS",
  description:
    "Онлайн-стенд MOMO и ZEUS: опубликованные записи моделей и демонстрация плеера. Для моделей без записи доступен только общий демотрек.",
  alternates: { canonical: "/listening-stand" },
  openGraph: {
    title: "Онлайн-стенд MOMO и ZEUS",
    description: "Онлайн-стенд MOMO и ZEUS. Для моделей без собственной записи доступен общий демонстрационный трек.",
    url: "/listening-stand",
    type: "website",
...
export default async function ListeningStandPage({ searchParams }: { searchParams: Promise<{ product?: string }> }) {
  ...
  const recordingCount = products.filter(hasPublishedListeningAudio).length;
  return (
    <main className="mx-auto max-w-[1200px] px-4 py-10 sm:px-6 sm:py-14">
      <section className="relative overflow-hidden rounded-[28px] border border-border bg-[#111214] px-5 py-8 text-white sm:px-8 sm:py-10 lg:px-12">
        ...
        <div className="relative max-w-[760px]">
          ...
          <h1 className="mt-4 font-display text-[clamp(2rem,6vw,4.4rem)] font-black uppercase leading-[0.96] tracking-[-0.055em]">
            Онлайн-стенд<br /><span className="text-[#ff5500]">{recordingCount ? "записи моделей" : "демоверсия"}</span>
          </h1>
          <p className="mt-5 max-w-[62ch] text-sm leading-relaxed text-white/62 sm:text-base">
            ...
            {recordingCount === 0 && " Собственные записи пока не опубликованы. Сейчас можно познакомиться с плеером на общем демонстрационном треке."}
          </p>
          <div className="mt-6 flex flex-wrap gap-x-6 gap-y-2 text-[0.7rem] font-semibold uppercase tracking-[0.14em] text-white/45">
            ...
            <span>{recordingCount} записей опубликовано</span>
      ...
      <ListeningStand products={products} initialSlug={initialSlug} />
    </main>
  );
}
