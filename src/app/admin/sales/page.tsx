...
export default async function SalesPage({ searchParams }: { searchParams: Promise<{ from?: string; to?: string }> }) {
  ...
  return <div>
    ...
    <h2 className="mt-9 font-display text-base font-extrabold uppercase">Товары за период</h2>{report.top.length === 0 ? <p className="mt-4 text-[0.85rem] text-muted-foreground">Продаж за этот период нет.</p> : <><div className="mt-3 grid gap-2 sm:hidden">{report.top.map((item) => <Link key={`mobile-${item.slug}`} href={`/admin/products/${item.slug}`} className="rounded-xl border border-border bg-surface p-4"><span className="block font-medium">{item.title}</span><span className="mt-2 flex items-center justify-between gap-3 text-sm text-muted-foreground"><span>{item.qty} шт.</span><b className="text-foreground">{formatPrice(item.revenue)}</b></span></Link>)}</div><div className="mt-3 hidden overflow-x-auto sm:block"><table className="w-full min-w-[520px] text-[0.85rem]"><thead><tr className="border-b border-border text-left text-[0.72rem] uppercase tracking-wider text-muted-foreground"><th className="py-2 pr-3">Товар</th><th className="py-2 pr-3 text-right">Продано</th><th className="py-2 text-right">Сумма</th></tr></thead><tbody>{report.top.map((item) => <tr key={item.slug} className="border-b border-border"><td className="py-3 pr-3"><Link href={`/admin/products/${item.slug}`} className="font-medium hover:text-signal">{item.title}</Link></td><td className="py-3 pr-3 text-right">{item.qty} шт.</td><td className="py-3 text-right font-semibold">{formatPrice(item.revenue)}</td></tr>)}</tbody></table></div></>}
  </div>;
}
