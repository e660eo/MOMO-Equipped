...
import { getDealerApplications, getDealerOrders } from "@/lib/dealers";
import { getDealerOrderAgreementVersions } from "@/lib/dealer-order-management";
import { matchesDealerView } from "@/lib/dealer-order-worklist";
...
export default async function AdminHomePage() {
  ...
  const dealerAgreements = new Map(getDealerOrderAgreementVersions().map((item) => [item.orderId, item]));
  const dealerToday = dealerOrders.filter((order) => moscowDay(order.createdAt) === today && order.status !== "canceled");
  const dealerMetrics = [
    { label: "Новые заказы", value: String(dealerOrders.filter((order) => order.status === "new").length), href: "/admin/dealers/orders?view=new" },
    { label: "Ждут оплаты", value: String(dealerOrders.filter((order) => matchesDealerView(order, dealerAgreements.get(order.id), "awaiting-payment")).length), href: "/admin/dealers/orders?view=awaiting-payment" },
    { label: "К отгрузке", value: String(dealerOrders.filter((order) => matchesDealerView(order, dealerAgreements.get(order.id), "ready")).length), href: "/admin/dealers/orders?view=ready" },
    { label: "Сумма заявок сегодня", value: formatPrice(dealerToday.reduce((sum, order) => sum + (dealerAgreements.get(order.id)?.total ?? order.total), 0)), href: "/admin/dealers/orders" },
    { label: "Заявки на дилерство", value: String(getDealerApplications().filter((item) => item.status === "new" && !item.archivedAt).length), href: "/admin/dealers#dealer-applications" },
  ];
  ...
  return <div>
    ...
    <section className="mt-7"><h2 className="font-display text-xl font-extrabold uppercase">Дилеры</h2><p className="mt-1 text-sm text-muted-foreground">Отдельно от розницы. Сумма заявок учитывает согласованные условия и не равна полученным оплатам.</p><div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{dealerMetrics.map((metric) => <Link key={metric.label} href={metric.href} className="rounded-xl border border-border bg-surface p-5 hover:border-signal"><p className="font-display text-2xl font-extrabold tabular-nums">{metric.value}</p><p className="mt-1 text-sm">{metric.label}</p></Link>)}</div></section>
