        ...
        {accounts.map((account) => {
          ...
          return <article key={`mobile-${account.id}`} className="rounded-xl border border-border p-4">
            ...
            <form action={setDealerTerms} className="mt-4 grid grid-cols-2 gap-3 border-t border-border pt-4">
              ...
              <label className="col-span-2 grid gap-1 text-xs text-muted-foreground">Ценовой уровень<select className={`${inputClass} w-full`} name="priceTier" defaultValue={tier}><option value="dealer">{DEALER_PRICE_TIER_LABELS.dealer}</option><option value="dagestan">{DEALER_PRICE_TIER_LABELS.dagestan}</option><option value="wholesale">{DEALER_PRICE_TIER_LABELS.wholesale}</option></select></label>
              <label className="grid gap-1 text-xs text-muted-foreground">Скидка, %<input className={`${inputClass} w-full`} type="number" min="0" max="80" step="0.1" name="discountPercent" defaultValue={account.discountPercent} /></label>
