...
export default async function AdminDealersPage({ searchParams }: { searchParams: Promise<{ application?: string }> }) {
  ...
  return <div><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.15em] text-signal">B2B</p><h1 className="mt-1 font-display text-3xl font-extrabold uppercase">Дилеры</h1><p className="mt-2 text-sm text-muted-foreground">Заявки, доступы, закрытые прайсы и дилерские заказы.</p>{priceBook.updatedAt && <p className="mt-2 text-xs text-muted-foreground">Единый дилерский прайс обновлён {new Date(priceBook.updatedAt).toLocaleDateString("ru-RU")} · {priceCounts.dealer} позиций</p>}</div><div className="flex gap-2 text-xs"><span className="rounded-full bg-signal/10 px-3 py-1.5 font-bold text-signal">{applications.filter((item) => item.status === "new" && !item.archivedAt).length} новых заявок</span><span className="rounded-full bg-black/5 px-3 py-1.5 font-bold">{orders.filter((item) => item.status === "new").length} новых заказов</span></div></div>
    <div className="mt-5"><Link href="/admin/dealer-prices" className="inline-flex min-h-11 items-center rounded-lg bg-signal px-4 text-sm font-bold text-white">Обновить дилерский прайс</Link></div>
    ...
    <section className="mt-5 rounded-xl border border-border bg-surface p-5">
      <div className="flex items-center gap-3"><Package className="text-signal" size={21} /><div><h2 className="font-display text-lg font-extrabold uppercase">Дилерские заказы</h2><p className="text-xs text-muted-foreground">Не смешиваются с розничными заказами.</p></div></div>
