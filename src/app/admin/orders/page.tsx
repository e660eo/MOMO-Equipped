        ...
        {orders.map((order) => (
          <article key={`mobile-${order.id}`} className="rounded-xl border border-border bg-surface p-4">
            <div className="flex items-start gap-3">
              <input type="checkbox" name="ids" value={order.id} aria-label={`Выбрать заказ ${order.id}`} className="mt-0.5 h-5 w-5 shrink-0" />
              <div className="min-w-0 flex-1">
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <div>
                    <Link href={`/admin/orders/${order.id}`} className="font-mono text-sm font-semibold text-signal">{order.id}</Link>
                    <p className="mt-1 text-xs text-muted-foreground">{dateFmt(order.createdAt)}</p>
                  </div>
                  <span className={cn("inline-block whitespace-nowrap rounded-full border px-2.5 py-1 text-[0.72rem]", STATUS_STYLE[order.status])}>{STATUS_LABELS[order.status]}</span>
                </div>
                <div className="mt-3 border-t border-border pt-3">
                  <p className="font-medium">{order.customer.name}</p>
                  <a href={`tel:${order.customer.phone.replace(/[^+\d]/g, "")}`} className="mt-1 inline-flex min-h-11 items-center text-sm text-muted-foreground">{order.customer.phone}</a>
                  <p className="text-[0.78rem] leading-relaxed text-muted-foreground">{order.items.map((item) => `${item.title} × ${item.qty}`).join(", ")}</p>
                </div>
                <div className="mt-3 flex items-end justify-between gap-3 border-t border-border pt-3">
                  <span className="text-xs text-muted-foreground">{order.delivery?.shipment?.status === "failed" ? "Ошибка доставки Ozon" : order.payment ? PAYMENT_LABELS[order.payment.status] : "Без онлайн-оплаты"}</span>
                  <b className="whitespace-nowrap font-display text-base">{formatPrice(order.total)}</b>
                </div>
              </div>
            </div>
          </article>
