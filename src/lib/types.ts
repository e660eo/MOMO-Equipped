...
export interface Promo {
  ...
  /**
   * Активные погашения по уникальному идентификатору заказа/попытки.
   * Нужны, чтобы отмена и повторный webhook были идемпотентными.
   */
...
export interface NewsItem {
   ...
   */
  body?: string;
  /** Товары из этой публикации: показываются под текстом статьи. */
  productSlugs?: string[];
}
...
export interface Order {
  ...
  promo?: {
    ...
    percent: number;
    discount: number;
    /** Уникальная операция погашения промокода. */
    redemptionId?: string;
    /** Ключ лимита на покупателя: customerId либо нормализованный телефон. */
    customerKey?: string;
    /** Счётчики промокода сейчас удерживают эту активацию. */
  ...
  note?: string;
  /**
   * Зарезервирован ли учитываемый остаток по этому заказу. Резерв создаётся
   * вместе с заказом и освобождается при отмене или неуспешной оплате; флаг
   * не даёт списать или вернуть товар дважды.
   */
